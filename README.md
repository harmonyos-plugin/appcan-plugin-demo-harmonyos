...
插件配置机制从`plugin.json`迁移到`plugin.config.ts`的说明，请参考:[插件配置机制迁移指南](./docs/插件配置机制迁移指南.md)
...
3. 在entry模块的src/main/ets/AppCanExPluginProvider.ets中导入插件配置（配置内容查看插件工程目录下`uexDemo/plugin.config.ts`），并通过`@appcan/uexdemo`导入插件模块。
